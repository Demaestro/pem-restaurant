...
export function createStorage({
  ...
  if (supabaseUrl && supabaseServiceRoleKey) {
    ...
    return {
      ...
      async reserveMenuStock(orderItems) {
        const payload = normalizeStockReservationPayload(orderItems);
        if (payload.error) {
          return { ok: false, error: payload.error };
        }

        const { data, error } = await supabase.rpc("reserve_menu_stock", {
          items: payload.items,
        });
        ...
        const fallback = reserveStockInMemory(await this.getMenuItems(), payload.items);
        if (!fallback.ok) {
          return fallback;
        }
        await this.updateMenuItems(fallback.menuItems);
        return { ok: true };
      },
  ...
  return {
    ...
    async reserveMenuStock(orderItems) {
      const db = await readLocalDb(dbPath);
      const menuItems = Array.isArray(db.menuItems) && db.menuItems.length > 0 ? db.menuItems : defaultMenuItems;
      const reservation = reserveStockInMemory(menuItems, orderItems);
      if (!reservation.ok) {
        return reservation;
      }
      db.menuItems = reservation.menuItems;
      await writeLocalDb(dbPath, db);
      return { ok: true };
    },
    async getDeliveryZones() {
...
function normalizeStockReservationPayload(orderItems) {
  if (!Array.isArray(orderItems) || orderItems.length === 0) {
    return { error: "At least one order item is required.", items: [] };
  }
  ...
  for (const item of orderItems) {
    ...
    if (!Number.isInteger(itemId) || itemId <= 0 || !Number.isInteger(quantity) || quantity < 1) {
      return { error: "One of the selected menu items is no longer available.", items: [] };
    }
    quantityById.set(itemId, (quantityById.get(itemId) || 0) + quantity);
  }

  return {
    error: "",
    items: [...quantityById.entries()].map(([itemId, quantity]) => ({
      item_id: itemId,
      quantity,
    })),
  };
...
function reserveStockInMemory(menuItems, orderItems) {
  const payload = normalizeStockReservationPayload(orderItems);
  if (payload.error) {
    return { ok: false, error: payload.error };
  }
  ...
  for (const item of payload.items) {
    ...
    if (menuItem.hidden || menuItem.soldOut) {
      return { ok: false, error: `${menuItem.name} is currently unavailable. Please remove it and try again.` };
    }
    const stockQuantity = Number(menuItem.stockQuantity || 0);
    if (stockQuantity > 0 && Number(item.quantity || 0) > stockQuantity) {
      return { ok: false, error: `${menuItem.name} only has limited stock left right now.` };
    }
  }

  const quantityById = new Map(payload.items.map((item) => [Number(item.item_id), Number(item.quantity) || 0]));
  const nextMenuItems = (menuItems || []).map((menuItem) => {
    const orderedQuantity = quantityById.get(Number(menuItem.id)) || 0;
    const stockQuantity = Number(menuItem.stockQuantity || 0);
    if (!orderedQuantity || stockQuantity <= 0) {
      return menuItem;
    }
    const remainingStock = Math.max(0, stockQuantity - orderedQuantity);
    return {
      ...menuItem,
  ...
  return { ok: true, error: "", menuItems: nextMenuItems };
}
